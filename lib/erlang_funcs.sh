...
function download_erlang() {
  ...
  if [ ! -f "$(erlang_download_path)/$(erlang_stack_version).tar.gz" ]; then
    ...
    curl -L -s ${erlang_package_url} -o "$(erlang_download_path)/$(erlang_stack_version).tar.gz" || exit 1
  else
...
function install_erlang() {
  ...
  tar zxf "$(erlang_download_path)/$(erlang_stack_version).tar.gz" -C $(erlang_build_path) --strip-components=1
